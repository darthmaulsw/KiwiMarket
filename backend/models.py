...
class Bounty(Base):
    ...
    fulfiller_wallet = Column(String, nullable=True)
    ...
    proofs = relationship("Proof", back_populates="bounty", cascade="all, delete-orphan")
...
class Proof(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    bounty_id = Column(Integer, ForeignKey("bounties.id"), nullable=False)
    fulfiller_wallet = Column(String, nullable=False)
    image_base64 = Column(Text, nullable=False)
    verdict = Column(String, nullable=True)     # "YES" or "NO"
    reasoning = Column(String, nullable=True)
    status = Column(String, nullable=False, default="pending")  # pending/verified/rejected
    created_at = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))

    bounty = relationship("Bounty", back_populates="proofs")
